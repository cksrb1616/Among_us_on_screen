...
class Sticker(QtWidgets.QMainWindow):
    ...
    def setupUi(self):
        ...
        w = int(movie.frameRect().size().width() * self.size)/2
        h = int(movie.frameRect().size().height() * self.size)/2
...
if __name__ == '__main__':
    ...
    s3 = Sticker('gif/amongus/blue_green.gif', xy=[300, 660], size=1.0, on_top=True)

    s4 = Sticker('gif/amongus/mint.gif', xy=[500, 475], size=0.2, on_top=True)
    s4.walk_diff(from_xy_diff=[-100, 0], to_xy_diff=[100, 0], speed=120)

    s5 = Sticker('gif/amongus/brown.gif', xy=[100, 505], size=0.75, on_top=True)

    s6 = Sticker('gif/amongus/yellow.gif', xy=[1100, 400], size=0.75, on_top=True)
    # s6.walk(from_xy=[0, 800], to_xy=[1850, 800], speed=240)

    s7 = Sticker('gif/amongus/magenta.gif', xy=[750, 450], size=0.5, on_top=True)
